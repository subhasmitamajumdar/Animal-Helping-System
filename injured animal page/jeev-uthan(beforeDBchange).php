 ...
 *{padding:0px; margin:0px;}
 ...
 <img src="<?php echo get_template_directory_uri();?>/page-templates/capimage.jpg">
    ...
    <?php
         global $wpdb; 
         $qu = $wpdb->get_results("(SELECT vet_location FROM wp_ju_vets_info GROUP BY vet_location) UNION 
                 (SELECT ngo_location FROM wp_ju_ngo_info GROUP BY ngo_location) order by vet_location asc");
    ?>
    ...
    <option value="<?php echo $b->vet_location;?>">
    <?php        
         echo $b->vet_location."<br>";
    ?>
    </option>
    <?php 
     } 
      ...
      /* @var $_POST type */
       if(($_POST['Location'])!=="" && $_POST['enter_captcha'] !=="")
       {
         ...
         if($_SESSION['rand_code']==$cap)
         {
          $loc = $_POST['Location'];
          global $wpdb;
          $d = $wpdb->get_results("select * FROM wp_ju_vets_info WHERE vet_location='$loc'");
          $d1 =$wpdb->get_results("select * FROM wp_ju_ngo_info WHERE ngo_location='$loc'");
